<template>
    <div class="container mt-10  px-8  ">
      ...
      <div class="mb-6 text-black text-2xl mx-2 font-semibold">How to get to Mizoram</div>

        <div>
            <router-link 
        :to="{ name: 'distanceCreate' }"
                          ...
                          <tr v-for="(value,index) in data" :key="value.name">
                              ...
                              <td
                                  class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                                   <router-link :to="{ name:'distanceEdit',params:{id:value.id}}" class="text-indigo-400 hover:text-indigo-900" > Edit</router-link>
                              </td>
                          </tr>
                      </tbody>
                  </table>
...
export default {
  ...
  created() {
    ...
    axios.get("/api/distances").then((result) => {
