<template>
    <div class="">
        ...
        <div class="bg-white border border-gray-300 ml-5 p-8 px-24 w-200">
            <div class="flex">
                <div class="mb-6 text-black text-2xl mx-2 font-semibold">Edit Carousal</div>
                <div class="col-end-3">
                    ...
                    <div class="flex">
                        <div class="mb-3 xl:w-96">
                            <label for="images" class="form-label inline-block mb-2 text-gray-700">
                               Upload Image <span class="font-bold">(SELECT ONLY ONE IMAGE)</span> 
                            </label>
...
export default {
  ...
  data(){
    ...
    created() {
        ...
        this.mediaLink ='/api/media/carousal/'+this.id

        //GET THE Festival
        axios.get("/api/carousal/"+this.id).then((result) => {
    ...
    methods: {
        updateModel(){
            this.mCarousal.data = this.data
          
            this.mCarousal.addedMedia = this.addedMedia
            this.mCarousal.savedMedia = this.savedMedia
            this.mCarousal.deletedMedia = this.deletedMedia
            
            axios.put("/api/carousal/"+this.id,this.mCarousal).then((result) => {
        ...
        deleteModel(){
            if(confirm("Do you really want to delete?")){
                ...
                axios.delete("/api/carousal/"+this.id).then((result) => {
                console.log(result.data.name);
                // this.data = result.data.data;
                if(result.status==200){
                    this.$router.push('./')
